...
class TrendLinePattern(BasePattern):
    ...
    def __init__(self, window: int = 50, pivot_k: int = 2, **kwargs) -> None:
        ...
        # lock 시점에 H2 타임스탬프를 직접 저장 (history 인덱스 부패 방지)
        self._lck_l2_ts:   datetime | None = None
        self._frozen:      bool            = False  # 포지션 중 채널 고정
    ...
    def evaluate(self, data: MarketData) -> PatternResult:
        ...
        if self._locked:
            ...
            if not self._frozen and (data.close > self._lck_upper or data.close < self._lck_lower):
                # 봉마감 채널 이탈 → H2 carry 후 무효화 (포지션 중엔 스킵)
                self._save_carry_from_locked()
                self._locked           = False
                self._lck_channel      = None
                self.downtrend_channel = None
        ...
        if self.downtrend_channel is not None:
            ...
            self._lck_l2_ts   = self._history[ch.l2_idx].timestamp
    ...
    def _save_carry_from_locked(self) -> None:
        ...
        if self._lck_channel is None:
            return
        self._carry_ts    = self._lck_l2_ts
...
class TrendLinePatternUp(BasePattern):
    """
    피벗 포인트로 상승 채널을 구성하고 잠근다.

    uptrend:
        lower  ─── L1 → L2  (pivot low 2개, slope > 0)
        upper  ─ ─ parallel, H1 (최고 피벗 고점) 통과

    채널 이탈(봉마감 기준) 시 L2를 새 L1으로 carry → 연속 채널 탐지.
    포지션 완전 청산(unlock) 시에도 L2 carry.

    필드 매핑 (TrendChannel 재사용):
      l1_idx/l1_price = L1 (첫 번째 저점)
      l2_idx/l2_price = L2 (두 번째 저점)
      h1_idx/h1_price = H1 (최고 고점)
    """
    ...
    def unlock(self) -> None:
        """포지션 완전 청산 후 호출. L2를 새 L1으로 carry."""
        self._frozen = False
        self._save_carry_from_locked()
        self._locked      = False
        self._lck_channel = None
    ...
    def evaluate(self, data: MarketData) -> PatternResult:
        ...
        if self._locked:
            ...
            # frozen 상태(포지션 중)에는 가격 이탈해도 채널 유지
            if not self._frozen and (data.close > self._lck_upper or data.close < self._lck_lower):
                self._save_carry_from_locked()
                self._locked      = False
                self._lck_channel = None
                ...
                self.uptrend_channel = self._lck_channel

            detected = self.uptrend_channel is not None
            return PatternResult(detected=detected, direction=None,
                                 strength=0.0, name=self.name)
        ...
        if self.uptrend_channel is not None:
            ch = self.uptrend_channel
            self._locked      = True
            self._lck_upper   = ch.upper_now
            self._lck_lower   = ch.lower_now
            self._lck_slope   = ch.slope
    ...
    def _build_uptrend_channel(
        ...
        # carry 모드: 이전 L2를 L1으로 고정, 이후 H1/L2만 탐색
        if self._carry_ts is not None:
            l1_idx = self._find_carry_in_history()
            if l1_idx is None:
                self._carry_ts = None
            else:
                highs_after = [i for i in pivot_highs if i > l1_idx]
                if not highs_after:
                    return None
                h1_idx   = max(highs_after, key=lambda i: self._history[i].high)
                h1_price = self._history[h1_idx].high
                ...
                return self._assemble_up_channel(
                    l1_idx, self._carry_price,
                    h1_idx, h1_price,
                    l2_idx, l2_price,
                    n,
                )
        ...
        return self._assemble_up_channel(
            l1_idx, l1_price, h1_idx, h1_price, l2_idx, l2_price, n
        )

    def _assemble_up_channel(
        self,
        l1_idx: int, l1_price: float,
        h1_idx: int, h1_price: float,
        l2_idx: int, l2_price: float,
